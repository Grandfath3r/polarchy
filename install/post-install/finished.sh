...
if [[ -f $POLARCHY_INSTALL_LOG_FILE ]] && grep -q "Total:" "$POLARCHY_INSTALL_LOG_FILE" 2>/dev/null; then
  echo
  TOTAL_TIME=$(tail -n 20 "$POLARCHY_INSTALL_LOG_FILE" | grep "^Total:" | sed 's/^Total:[[:space:]]*//')
...
if sudo test -f /etc/sudoers.d/99-polarchy-installer; then
  sudo rm -f /etc/sudoers.d/99-polarchy-installer &>/dev/null
fi
