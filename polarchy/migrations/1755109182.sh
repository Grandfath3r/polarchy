...
# Users can still opt-in to Cloudflare DNS using: polarchy-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-polarchy-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-polarchy-dns.network
  ...
  echo "To use Cloudflare DNS, run: polarchy-setup-dns Cloudflare"
fi