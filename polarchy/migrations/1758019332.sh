...
polarchy-refresh-config uwsm/default
polarchy-refresh-config uwsm/env
polarchy-state set reboot-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, polarchy-launch-editor' ~/.config/hypr/bindings.conf
fi
