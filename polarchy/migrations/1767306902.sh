...
# Move user-added backgrounds from Polarchy theme folders to user config
POLARCHY_DIR="$HOME/.local/share/polarchy"
USER_BACKGROUNDS_DIR="$HOME/.config/polarchy/backgrounds"

if [[ -d "$POLARCHY_DIR/themes" ]]; then
  cd "$POLARCHY_DIR"

  # Get list of git-tracked background files (relative to polarchy dir)
...
THEMES_DIR="$HOME/.config/polarchy/themes"
CURRENT_THEME_LINK="$HOME/.config/polarchy/current/theme"
...
elif [[ -f "$HOME/.config/polarchy/current/theme.name" ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/polarchy/current/theme.name")
fi

# Remove all symlinks from ~/.config/polarchy/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  polarchy-theme-set "$CURRENT_THEME_NAME"
else
  # Backup to ensure a theme is set if we can't deduce the name
  polarchy-theme-set "Tokyo Night"
fi
