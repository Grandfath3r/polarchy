...
EOF
  ...
  sudo cp $POLARCHY_PATH/default/limine/default.conf /etc/default/limine
  ...
  sudo cp $POLARCHY_PATH/default/limine/limine.conf /boot/limine.conf


  # Match Snapper configs if not installing from the ISO
  if [[ -z ${POLARCHY_CHROOT_INSTALL:-} ]]; then
...
#
#   uki_file=$(find /boot/EFI/Linux/ -name "polarchy*.efi" -printf "%f\n" 2>/dev/null | head -1)
#
#   if [[ -n "$uki_file" ]]; then
